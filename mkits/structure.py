...
class struct(object):
    ...
    def write_struct(self, 
        ...
        # ================== CIF SUPPORT ==================
            ...
            # 1. Header & Symmetry
            safe_title = self.title.replace(" ", "_") if self.title != "none" else "generated_structure"
            _lines.append(f"data_{safe_title}\n")
            _lines.append("_audit_creation_method   'Generated by Struct Class'\n\n")
            
            # Default P1 symmetry
            _lines.append("_symmetry_space_group_name_H-M    'P 1'\n")
            _lines.append("_symmetry_Int_Tables_number       1\n")
            _lines.append("_symmetry_cell_setting            triclinic\n\n")
            
            # 2. Cell Parameters
            # lattice6: [a, b, c, alpha, beta, gamma]
            if len(self.lattice6) == 6:
                a, b, c, alpha, beta, gamma = self.lattice6
            else:
                # if lattice6 is void, try lattice9 
                self.lattice6 = functions.lattice_conversion(self.lattice9)
                a, b, c, alpha, beta, gamma = self.lattice6

            _lines.append(f"_cell_length_a        {a:12.6f}\n")
            _lines.append(f"_cell_length_b        {b:12.6f}\n")
            _lines.append(f"_cell_length_c        {c:12.6f}\n")
            _lines.append(f"_cell_angle_alpha     {alpha:12.6f}\n")
            _lines.append(f"_cell_angle_beta      {beta:12.6f}\n")
            _lines.append(f"_cell_angle_gamma     {gamma:12.6f}\n\n")
            
            # 3. Atomic Positions Loop
            _lines.append("loop_\n")
            _lines.append(" _atom_site_label\n")
            _lines.append(" _atom_site_type_symbol\n")
            _lines.append(" _atom_site_fract_x\n")
            _lines.append(" _atom_site_fract_y\n")
            _lines.append(" _atom_site_fract_z\n")
            
            # 这里的逻辑是生成唯一的标签，例如 C1, C2, H1, H2
            element_counter = {}
            
            # 强制使用分数坐标 (CIF 标准)
            frac_coords = self.position[1:, 1:4]
            
            for i, sym in enumerate(_atomic_symbols):
                # 计数生成 label
                if sym not in element_counter:
                    element_counter[sym] = 1
                else:
                    element_counter[sym] += 1
                label = f"{sym}{element_counter[sym]}"
                
                fx, fy, fz = frac_coords[i]
                
                # 格式化写入
                _lines.append(f" {label:<8s} {sym:<4s} {fx:12.8f} {fy:12.8f} {fz:12.8f}\n")
